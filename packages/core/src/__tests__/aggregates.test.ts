...
describe('Aggregation Functions', () => {
  ...
  beforeEach(async () => {
    ...
    await db.insert('orders').values({
      id: 1, userId: 'user1', amount: 100, status: 'completed', createdAt: '2024-01-01'
    });
    await db.insert('orders').values({
      id: 2, userId: 'user1', amount: 200, status: 'completed', createdAt: '2024-01-02'
    });
    await db.insert('orders').values({
      id: 3, userId: 'user2', amount: 150, status: 'completed', createdAt: '2024-01-03'
    });
    await db.insert('orders').values({
      id: 4, userId: 'user2', amount: 50, status: 'pending', createdAt: '2024-01-04'
    });
  });
  ...
  describe('avg()', () => {
    ...
    it('should calculate average with WHERE clause', async () => {
      const average = await db.query('orders')
        .where('userId', '=', 'user1')
        .avg('amount');
  ...
  describe('min()', () => {
    ...
    it('should find minimum with WHERE clause', async () => {
      const min = await db.query('orders')
        .where('status', '=', 'completed')
        .min('amount');
      expect(min).toBe(100);
    });
  ...
  describe('max()', () => {
    ...
    it('should find maximum with WHERE clause', async () => {
      const max = await db.query('orders')
        .where('userId', '=', 'user2')
        .max('amount');
      expect(max).toBe(150);
    });
